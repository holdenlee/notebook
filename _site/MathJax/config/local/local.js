MathJax.Hub.Config({
  TeX: {
    ...
    Macros: {
      ...
      nv: ["\\frac{#1}{\\left\\Vert {#1}\\right\\Vert}",1],
      nvl: ["\\frac{#1}{\\left\\Vert {#1}\\right\\Vert_{#2}}",2],
      rb: ["\\left.{#1}\\right|",1],
      ...
      maxr: ["\\max_{\\scriptsize \\begin{array}{c}{#1}\\\\{#2}\\end{array}}",2],
      minr: ["\\min_{\\scriptsize \\begin{array}{c}{#1}\\\\{#2}\\end{array}}",2],
      trow: ["\\scriptsize \\begin{array}{c}{#1}\\\\{#2}\\end{array}",2],
      su: "\\sum_{n=0}^{\\iy}",
      suo: "\\sum_{n=1}^{\\iy}",
      sumo: ["\\sum_{#1=1}^{#2}",2],
      sumz: ["\\sum_{#1=0}^{#2}",2],
      prodo: ["\\prod_{#1=1}^{#2}",2],
      prodz: ["\\prod_{#1=0}^{#2}",2],
      oplo: ["\\bigopl_{#1=1}^{#2}",2],
      cupo: ["\\bigcup_{#1=1}^{#2}",2],
      capo: ["\\bigcap_{#1=1}^{#2}",2],
      iiy: "\\int_0^{\\infty}",
      ...
      colfour: [" \\begin{pmatrix} {#1}\\\\ {#2}\\\\ {#3}\\\\ {#4} \\end{pmatrix} ",4],
      ...
      mattn: [" \\begin{pmatrix} {#1}&{#2}&{#3}\\\\ {#4}&{#5}&{#6}\\\\ {#7}&{#8}&{#9} \\end{pmatrix} ",9],
      bt: [" \\left\\{\\begin{matrix} \\text{#1}\\\\ \\text{#2} \\end{matrix} \\right\\}",2],
      bth: [" \\left\\{\\begin{matrix} \\text{#1}\\\\ \\text{#2}\\\\ \\text{#3} \\end{matrix} \\right\\}",3],
      tcase: [" \\begin{cases} #1 & #2\\\\ #3 & #4 \\end{cases}",4],
      txcase: [" \\begin{cases} #1 & \\text{#2}\\\\ #3 & \\text{#4} \\end{cases}",4],
      ifoth: [" \\begin{cases} #1 & \\text{if }#2\\\\ #3 & \\text{otherwise} \\end{cases}",3],
      ...
      MAJ: "\\operatorname{\\mathsf{Maj}}",
      Parity: "\\operatorname{\\mathsf{Parity}}",
