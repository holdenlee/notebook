...
readFields contents = deps $ fromRight (parse (fieldsParser emptyPI) "error" contents)
...
chain inputFs init action parser = 
  do 
    handles <- sequence (fmap (\x -> openFile x ReadMode) inputFs)
    contents <- sequence (fmap hGetContents handles)
    let pi = foldIterate2 (\fileName text pi -> fromRight (parse (parser (action pi fileName)) "error" text)) inputFs contents init
    return pi
